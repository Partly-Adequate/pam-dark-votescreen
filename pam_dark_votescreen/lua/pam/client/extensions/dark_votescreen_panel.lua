...
-- images and icons
local ic_favorite = Material("vgui/pam_dark_votescreen/ic_favorite")
local ic_not_favorite = Material("vgui/pam_dark_votescreen/ic_not_favorite")
local ic_voted_on = Material("vgui/pam_dark_votescreen/ic_selected")
local ic_not_voted_on = Material("vgui/pam_dark_votescreen/ic_not_selected")

local mat_missing_option = Material("vgui/pam_dark_votescreen/img_missing")
...
function PANEL:RefreshOptionList()
	...
	for _, option_button in pairs(self.option_buttons) do
		if (not PAM.winning_option and self:FitsSearchTerm(option_button) and (not self.show_favorites or PAM.IsFavorite(option_button.option.name)) and (not self.show_voted_on or option_button.voter_count > 0)) or PAM.winning_option.id == option_button.option.id then
